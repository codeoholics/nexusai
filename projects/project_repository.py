...
def find_similar_and_check_plagiarism(embedding_type, embedding):
    ...
    FROM (
        ...
        FROM project_vectors where embedding_type = %s
    ) AS subquery
    WHERE similarity < %s
    ORDER BY similarity
    LIMIT %s
     ...
     """,
                    (embedding,embedding_type, max_acceptable_distance, top_n)
                )
...
def update_project_plagiarism_details(project_id, plagiarism_details):
    try:
        # Convert the array of dictionaries into a JSONB format
        plagiarism_details_jsonb = json.dumps(plagiarism_details)

        cursor = db_client.get_conn().cursor()

        update_query = """UPDATE projects
                          SET plagiarism_details = %(plagiarism_details)s
                          WHERE id = %(project_id)s
                          RETURNING *;"""

        update_params = {
            'plagiarism_details': plagiarism_details_jsonb,
        ...
        raise Exception(f"Error updating plagiarism details in database: {e}")
